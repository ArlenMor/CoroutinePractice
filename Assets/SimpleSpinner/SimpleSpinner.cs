...
namespace Assets.SimpleSpinner
{
    ...
    public class SimpleSpinner : MonoBehaviour
    {
        ...
        public void Update()
        {
            if (Rotation)
            {
                transform.localEulerAngles = new Vector3(0, 0, -360 * RotationAnimationCurve.Evaluate((RotationSpeed * UnityEngine.Time.time + _period) % 1));
            }

            if (Rainbow)
            {
                _image.color = Color.HSVToRGB(RainbowAnimationCurve.Evaluate((RainbowSpeed * UnityEngine.Time.time + _period) % 1), RainbowSaturation, 1);
            }
        }
    }
}